...
const SIGNUP_MUTATION = gql`
	mutation SIGNUP_MUTATION(
		...
		signup(email: $email, name: $name, password: $password, allowMailchimp: $allowMailchimp) {
			id
			email
			name
		}
...
let SignupNew = () => {
	...
	let [selectedPlan, updateSelectedPlan] = useState(handlePlanPreselect(router.query.plan || null));
				...
				<div className="w-full max-w-4xl px-4 space-y-6 md:space-y-0 md:space-x-8 md:flex">
					<div className="pb-6 space-y-2 border-b md:py-6 md:justify-between md:flex md:flex-col md:space-y-2 md:border-b-0 md:w-1/3 md:text-right">
						<div>
							...
							<h1 className="ds-heading text-ds-primary">Sign Up & Get Started</h1>
					...
					>
						...
						<div className="space-y-2">
							...
							>
								...
								<p className="text-sm bg-ds-gray">
									Yes, please send me emails about Platform updates, tutorials, user guides and
									more.
								</p>
							</div>
							<div
							...
							>
								<div className="pt-1">
									<input checked={fakeInput} className="hidden" readOnly type="checkbox" />
									<Checkbox />
								</div>
								<p className="text-sm bg-ds-gray">
									We put this box here to see if you’re paying attention or just like ticking off
									lists, like us.
								</p>
							</div>
						</div>
